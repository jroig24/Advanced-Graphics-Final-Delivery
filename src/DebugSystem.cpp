...
void DebugSystem::updateimGUI_(float dt) {
	...
	if (show_imGUI_)
	{
		...
		ImGui::Begin("Custom parameters", &show_imGUI_);
		
		//Light color change
		auto& lights = ECS.getAllComponents<Light>();
		bool updateLights = false;
		for (size_t i = 0; i < lights.size(); i++) {
			float light_color[3] = { lights[i].color.x, lights[i].color.y, lights[i].color.z };

			ImGui::SliderFloat3("Force Light Color", light_color, 0.0f, 1.0f, "%.3f");
			lights[i].color.x = light_color[0];
			lights[i].color.y = light_color[1];
			lights[i].color.z = light_color[2];

			if (!updateLights && lights[i].color.value_ != light_color) {
				updateLights = true;
			}
		}
		...
		//Daylight
		float time[1] = { ECS.time_velocity };
		ImGui::SliderFloat("Time velocity", time, 0, 1);
		ECS.time_velocity = time[0];
		
		static bool shouldTimeGo = ECS.time_is_going;
		ImGui::Checkbox("Do advance in time", &shouldTimeGo);
		if (shouldTimeGo) {
			ECS.time_is_going = true;
			ECS.time_in_day += dt * ECS.time_velocity;
			ECS.time_in_day -= (ECS.time_in_day >= 1) ? 1 : 0;
			float exp = 0.7f;
			float lerpValue = sin(2*3.14159265 * ECS.time_in_day);
			lerpValue = (lerpValue < 0) ? -pow(abs(lerpValue), exp) : pow(lerpValue, exp);
			lerpValue = lerpValue * 0.5f + 0.5f;

			lm::vec3 night_color = lm::vec3((float)19/255, (float)24/255, (float)98/255);
			lm::vec3 day_color = lm::vec3(1, 1, (float)251/255);

			lights[0].color = night_color.lerp(day_color, lerpValue);

			updateLights = true;
		}
		else {
			ECS.time_is_going = false;
		}
		...
		//Terrain
		int terrain_id = ECS.getEntity("Terrain");
		Mesh& TerrainMesh = ECS.getComponentFromEntity<Mesh>(terrain_id);
		Material& TerrainMat = graphics_system_->getMaterial(TerrainMesh.material);
		float spec[1] = { TerrainMat.specular.x };
		ImGui::SliderFloat("Terrain weatness", spec, 0.0,1.0);
		
		TerrainMat.specular.x = spec[0];
		TerrainMat.specular.y = spec[0];
		TerrainMat.specular.z = spec[0];

		Transform& TerrainTrans = ECS.getComponentFromEntity<Transform>(terrain_id);

		if (ImGui::Button("Rise sea level")) {
			TerrainTrans.translate(0, -0.05, 0);
		}

		if (ImGui::Button("Down sea level")) {
			TerrainTrans.translate(0, 0.05, 0);
		}
