...
void Game::init(int w, int h) {
	...
	mat_terrain.noise_map = Parsers::parseTexture("data/assets/terrain/Iceland.tga",&noise_image_data,true);
	//mat_terrain.noise_map = Parsers::parseTexture("data/assets/terrain/island2.tga", &noise_image_data, true);
	...
	int sea_geometry = graphics_system_.createGeometryFromFile("data/assets/floor_40x40.obj");

	int sea_entity = ECS.createEntity("Sea");
	Mesh& sea_mesh = ECS.createComponentForEntity<Mesh>(sea_entity);
	sea_mesh.geometry = sea_geometry;
	sea_mesh.material = mat_blue_check_index;

	Transform& trans = ECS.getComponentFromEntity<Transform>(sea_entity);
	trans.scale(5,1,5);
	//trans.translate(0, 0.0f, 0);
